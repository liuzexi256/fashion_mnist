...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(784, 512) # 784表示输入神经元数量，1000表示输出神经元数量
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output = model(data)
        loss = F.cross_entropy(output, target)       # negative log likelihood loss(nll_loss), sum up batch cross entropy
        loss.backward()
...
def main():
    ...
    tr_data, val_data = torch.utils.data.random_split(train_data, [42000,18000])
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        validation_loss.append(test(args, model, device, validation_loader))

    np.save('train_loss_2fc_lr0.01.npy', np.array(train_loss))
    np.save('validation_loss_2fc_lr0.01.npy', np.array(validation_loss))
